...
def compute_token_to_index(vocabulary: Tuple[str], counts: Tuple[int]) -> Tuple[Dict[str, int], Dict[int, str], Dict[int, int]]:
    ...
    ###########################
    # YOUR CODE HERE
    tuple_length = len(vocabulary)
    idx = list(range(tuple_length))
    dict_1 = {k: v for k, v in zip(vocabulary, idx)}
    dict_2 = {k: v for k, v in zip(idx, vocabulary)}
    dict_3 = {k: v for k, v in zip(idx, counts)}
...
def get_token_pairs_from_window(sequence: Sequence[str], window_size: int, token_to_index: Dict[str, int]) -> Sequence[Tuple[int, int]]:
    ...
    ###########################
    # YOUR CODE HERE
    idx_list = [token_to_index[token] for token in sequence]
    pairs = []
    for i in range(len(idx_list)):
        context = idx_list[i-window_size:i] if i-window_size >= 0 else idx_list[0:i]
        context += idx_list[i+1:i+window_size+1]
        pairs += [(idx_list[i], context_idx) for context_idx in context]
