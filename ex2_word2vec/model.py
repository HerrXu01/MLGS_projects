...
class Embedding():
    ...
    def one_hot(self, sequence: NDArray, num_classes: int) -> NDArray:
        ...
        ###########################
        # YOUR CODE HERE
        one_hot = np.zeros((num_classes, sequence.shape[0]), dtype=np.int32)
    ...
    def softmax(self, x: NDArray, axis: int) -> NDArray:
        ...
        ###########################
        # YOUR CODE HERE
        y = x - x.max(axis=axis, keepdims=True) # stabilization
        y = np.exp(y) / np.sum(np.exp(y), axis=axis, keepdims=True)
        ###########################
        return y
    ...
    def loss(self, y_true: NDArray, y_predicted: NDArray) -> float:
        ...
        ###########################
        # YOUR CODE HERE
        M = y_true.shape[1]  # num_samples
        loss = (-1.0 / M) * np.sum(y_true * np.log(y_predicted))
        ###########################
    ...
    def forward(self, x: NDArray, y: NDArray) -> float:
        ...
        """
        x = self.one_hot(x, self.vocabulary_size)  # (N, t) where N = self.vocabulary_size, t is the sequence length
        ...
        ###########################
        # YOUR CODE HERE
        embedding = self.U @ x  # (D, t)
        logits = self.V @ embedding  # (N, t)
    ...
    def backward(self) -> Dict[str, NDArray]:
        ...
        # YOUR CODE HERE
        M = y.shape[1]
        d_V = (-1.0 / M) * (y - prob) @ embedding.T
        d_U = (-1.0 / M) * self.V.T @ (y - prob) @ x.T
        ###########################
