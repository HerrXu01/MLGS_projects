...
class NeuralTPP(torch.nn.Module):
    ...
    def __init__(self, hidden_dim: int = 16):
        ...
        self.embedding_rnn = torch.nn.RNN(2, self.hidden_dim, batch_first=True)
        #######################################################
    ...
    def encode(
        ...
        x = torch.cat((torch.unsqueeze(times, 2), torch.unsqueeze(log_times, 2)), dim=2)
        #######################################################
        return x
    ...
    def embed_history(
        ...
        #######################################################
        # write here and replace the default
        encode_data = self.encode(times)
        output, c_last = self.embedding_rnn(encode_data)
        # forgo c_last
        output_1 = output[:, :output.shape[1]-1, :]
        # add c_1
        batch = output.shape[0]
        max_seq_length = output.shape[2]
        history_emb = torch.cat((torch.zeros((batch, 1, max_seq_length)), output_1), dim=1)
        #######################################################
    ...
    def get_distribution_parameters(
        ...
        #######################################################
        # write here and replace the default
        params = self.linear(history_emb)
        split_params = torch.split(params, 1, dim=2)
        mu = split_params[0].squeeze()
        log_sigma = split_params[1].squeeze()
