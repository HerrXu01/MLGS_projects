...
class HMM_TxtGenerator:
    ...
    def forwards(self, x):
        ...
        ### YOUR CODE HERE ###
        alpha = np.zeros((K, T))
        alpha[:, 0] = pi.ravel() * B[:, x[0]]
        for i in range(1, T):
            alpha[:, i] = (A.T @ alpha[:, i-1]) * B[:, x[i]]
        alpha = alpha.T
        return alpha
    ...
    def generate_sentence(self, sentence_length):
        ...
        B = self.model_params.B  # [KxV]
        pi = self.model_params.pi  # [Kx1]
        Z_ss = np.arange(self.K) # sample space of latent variables
        X_ss = np.arange(self.V) # sample space of observations
        Z = []
        X = []
        # sample the first latent variable Z_0
        Z.append(np.random.choice(Z_ss, p=pi.ravel()))
        # sample the first observation
        X.append(np.random.choice(X_ss, p=B[Z[0], :]))
        # sample the remaining latent variables and observations
        for i in range(1, sentence_length):
            Z.append(np.random.choice(Z_ss, p=A[Z[-1], :]))
            X.append(np.random.choice(X_ss, p=B[Z[-1], :]))
